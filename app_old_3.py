...
with right_col:
    ...
    if st.session_state.skill_test_started:
        ...
        with st.form("skill_test_form"):
            ...
            if skill_mode == "Quick Recall":
                q1 = st.text_input("Question 1: Define the main concept from this project.")
                q2 = st.text_input("Question 2: Name one tool or framework used.")
                q3 = st.text_input("Question 3: What problem does this project solve?")

            elif skill_mode == "Concept Check":
                q1 = st.text_area("Explain a key concept from this project in your own words.")
                q2 = st.text_area("What is the difference between retrieval and generation?")
                q3 = st.text_area("Why does grounding matter in an LLM application?")

            else:  # Applied Reasoning
                q1 = st.text_area(
                    "Scenario: A user uploads new files into a project. Describe what the system should do next."
                )
                q2 = st.text_area(
                    "How would you evaluate whether answers from this project are improving over time?"
                )
                q3 = st.text_area(
                    "What would you change in the architecture to support multiple project-specific indexes?"
                )

            submit_skill_test = st.form_submit_button("Submit Skill Test")

            if submit_skill_test:
                st.session_state.skill_test_submitted = True

        if st.session_state.skill_test_submitted:
            st.success("Skill test submitted successfully. Demo scoring coming soon.")
            st.write("**Demo Summary**")
            st.write(f"Quiz style: {skill_mode}")
            st.write(
                f"Project: {current_project['name'] if current_project else 'No project selected'}"
            )
